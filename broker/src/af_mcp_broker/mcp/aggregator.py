...
def _make_client_factory(
    ...
    *,
    ...
    Trace-context propagation extends the same invariant: no inbound HTTP
    header -- ``traceparent`` included -- is ever forwarded to a service.
    The ``Client`` built here (fastmcp 3.4.4's ``Client.call_tool``) injects
    the broker's own then-current span context into the outbound MCP
    request's ``_meta`` (SEP-414), which is broker-generated by
    construction: the inbound ``_meta`` traceparent is only ever parsed as a
    remote parent of AuthorizationMiddleware's span (see mcp/middleware/
    authorization_mw.py), never copied forward verbatim.
    ...
    baked-in credential. A service whose tool list genuinely does personalize
    per caller should set ``ServiceSpec.tools_cache_ttl: 0`` to disable that
    cache outright.
    """
    transport_cls = SSETransport if spec.transport == "sse" else StreamableHttpTransport
...
def build_aggregator(
    ...
    audits every invocation) -- then adds one ProxyProvider per service in
    ``registry`` plus the broker-native af_* diagnostic tools (issue #153,
    see mcp/diagnostics.py). Namespacing follows ``ServiceSpec.apply_namespace``
    -- services whose tools already self-prefix (e.g. rucio-mcp) opt out.
