...
describe('linkKrb5Keytab', () => {
  it('POSTs username/keytab_b64 and returns ticket metadata', async () => {
    globalThis.fetch = mockJson(201, {
      target: 'condor',
      principal: 'auser@ATLAS.EXAMPLE.ORG',
      realm: 'ATLAS.EXAMPLE.ORG',
    ...
    const result = await linkKrb5Keytab('auser', 'a2V5dGFi');

    const [url, init] = vi.mocked(globalThis.fetch).mock.calls[0] as [string, RequestInit];
    expect(String(url)).toContain('/krb5/keytab');
    expect(init.method).toBe('POST');
    expect(JSON.parse(init.body as string)).toEqual({
      username: 'auser',
      keytab_b64: 'a2V5dGFi',
  ...
  it('includes target/lifetime/renewable_lifetime in the body when supplied', async () => {
    ...
    await linkKrb5Keytab('auser', 'a2V5dGFi', 'condor', '24h', '7d');
  ...
  it.each([400, 403, 422, 429, 502])(
    ...
    async (status) => {
      globalThis.fetch = vi
        .fn()
        .mockResolvedValue(new Response('nope', { status, statusText: 'Error' }));
      await expect(linkKrb5Keytab('auser', 'a2V5dGFi')).rejects.toMatchObject({
        name: 'APIError',
        status,
        body: 'nope',
      });
